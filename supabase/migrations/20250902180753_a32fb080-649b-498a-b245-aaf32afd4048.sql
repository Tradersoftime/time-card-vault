-- Fix admin_list_cards function to use correct table name and columns
...
-- Also fix the older version of admin_list_cards without p_include_deleted parameter
CREATE OR REPLACE FUNCTION public.admin_list_cards(p_search text DEFAULT NULL::text, p_limit integer DEFAULT 200, p_offset integer DEFAULT 0)
 RETURNS TABLE(id uuid, code text, name text, era text, suit text, rank text, rarity text, trader_value text, time_value numeric, image_url text, redirect text, is_active boolean, status text, created_at timestamp with time zone, owner_user_id uuid, owner_email text, is_in_pending_redemption boolean, is_credited boolean)
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
