-- Fix column ambiguity in admin_list_cards function by properly qualifying column names
...
begin
  ...
  where (p_search is null or p_search = '')
     or (base.code ilike '%'||p_search||'%' or coalesce(base.name,'') ilike '%'||p_search||'%' or coalesce(base.owner_email,'') ilike '%'||p_search||'%')
