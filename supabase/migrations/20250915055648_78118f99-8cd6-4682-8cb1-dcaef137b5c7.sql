-- Update admin_list_users to treat NULL, empty string, or 'all' as no filter
...
BEGIN
  ...
  WITH user_stats AS (
    SELECT 
      ...
      u.last_sign_in_at,
      COALESCE(uc.card_count, 0)::int AS total_cards_owned,
      COALESCE(pr.pending_count, 0)::int AS pending_redemptions,
      COALESCE(cr.credited_count, 0)::int AS credited_redemptions,
      COALESCE(se.scan_count, 0)::int AS total_scans,
